...
# Admins & Users
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '6476946240').split()] # For Multiple Id Use One Space Between Each.
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '6476946240').split()]  # For Multiple Id Use One Space Between Each.
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002512990126 -1002379554935').split()]  # For Multiple Id Use One Space Between Each.
...
auth_channel = environ.get('AUTH_CHANNEL', '-1002454650435') # give your force subscribe channel id here else leave it blank
...
DATABASE_NAME = environ.get('DATABASE_NAME', "moximovie1")
...
SHORTLINK_API = environ.get('SHORTLINK_API', 'f65177326ab770228557570f255893db54df50da')
